...
pub fn check_matrix_dimensions(
    ...
    let eq_x = a_length_x == res_length_x;
    let eq_y = a_length_y == b_length_y;
    let eq_z = b_length_z == res_length_z;
...
pub struct RepetitiveFreivald {
    xs: Vec<Freivald>,
}

impl RepetitiveFreivald {
    pub fn new(round: u32, array_size: usize) -> Self {
        let xs = (0..round).map(|_| Freivald::new(array_size)).collect();
        Self { xs }
    }
...
fn main() {
    ...
    let res = v.verify(mat_a, mat_b, mat_solution);
    println!("res: {}", res);
}
...
mod tests {
    ...
    fn multiple_freivald_verify_success_test(
        ...
        let round = 10;
        let multi_freivald = RepetitiveFreivald::new(round, supposed_ab.len());
        assert!(multi_freivald.multi_verify(
            matrix_a.clone(),
            matrix_b.clone(),
            supposed_ab.clone()
        ))
